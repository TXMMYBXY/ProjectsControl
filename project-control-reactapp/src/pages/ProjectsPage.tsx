...
import { projectsApi, GetProjectViewModel, CreateProjectViewModel, UpdateProjectViewModel } from '../api';
...
function getFullName(emp?: { firstName?: string | null; lastName?: string | null; patronymic?: string | null } | null): string {
  if (!emp) return '—';
  return [emp.lastName, emp.firstName, emp.patronymic].filter(Boolean).join(' ') || '—';
}

type SortField = 'title' | 'priority' | 'startDate';
...
export function ProjectsPage() {
  ...
  const [editProject, setEditProject] = useState<GetProjectViewModel | null>(null);
  const [viewProject, setViewProject] = useState<GetProjectViewModel | null>(null);
  const [docsProject, setDocsProject] = useState<GetProjectViewModel | null>(null);
  const [deleteProject, setDeleteProject] = useState<GetProjectViewModel | null>(null);
  ...
  const handleCreate = async (data: CreateProjectViewModel | UpdateProjectViewModel) => {
    await projectsApi.create(data as CreateProjectViewModel);
    showToast('Проект успешно создан');
    setCreateOpen(false);
    refetch();
  };

  const handleEdit = async (data: CreateProjectViewModel | UpdateProjectViewModel) => {
    if (!editProject) return;
    await projectsApi.update(editProject.id, data as UpdateProjectViewModel);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500 mt-1">{projects.length} проект{projects.length === 1 ? '' : projects.length < 5 ? 'а' : 'ов'}</p>
        </div>
        <button
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2">
          {(['title', 'priority', 'startDate'] as SortField[]).map(f => (
            <button
              key={f}
            ...
            >
              {{ title: 'Название', priority: 'Приоритет', startDate: 'Дата' }[f]}
              <SortIcon field={f} />
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(project => (
            <div key={project.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow p-5 flex flex-col gap-3">
              ...
              <div className="flex items-center gap-2 text-xs text-gray-500">
                <Users size={12} />
                <span>{project.employees?.length ?? 0} сотрудник{(project.employees?.length ?? 0) === 1 ? '' : (project.employees?.length ?? 0) < 5 ? 'а' : 'ов'}</span>
                {project.projectManager && (
                  <span className="text-gray-400">• РП: {getFullName(project.projectManager)}</span>
                )}
              </div>
      ...
      <Modal open={!!editProject} onClose={() => setEditProject(null)} title="Редактировать проект" size="lg">
        {editProject && (
          <ProjectForm mode="edit" initialData={editProject} employees={employees} onSubmit={handleEdit} onCancel={() => setEditProject(null)} />
        )}
      </Modal>
        ...
        {viewProject && (
          <div className="space-y-5">
            ...
            <div className="bg-gray-50 rounded-xl p-3">
              ...
              <Badge variant={priorityVariant(viewProject.priority)}>{priorityLabel(viewProject.priority)} ({viewProject.priority})</Badge>
            </div>
            <div>
        ...
        {deleteProject && (
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => setDeleteProject(null)} className="px-4 py-2.5 border border-gray-200 text-gray-600 text-sm rounded-lg hover:bg-gray-50 transition">
                Отмена
              </button>
            </div>
          </div>
